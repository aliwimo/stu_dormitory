...
namespace VeriTaban
{
    public partial class student_states : Form
    {
        ...
        public student_states()
        {
            InitializeComponent();
        }

        private void student_states_Load(object sender, EventArgs e)
        {
        ...
        public void refresh()
        {
            ...
            query = $"SELECT states.id AS ID, CONCAT(student.name, ' ', student.surname) AS 'Full Name', UPPER(states.state) AS 'IN / OUT', DATE_FORMAT(date, {date_format}) AS 'Time' FROM states, student WHERE states.personal = '0' AND states.id = student.student_id ORDER BY date DESC";
            this.paging(query);
        ...
        private void search_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                DBConnection con = new DBConnection();
                query = $"SELECT states.id AS ID, CONCAT(student.name, ' ', student.surname) AS 'Full Name', UPPER(states.state) AS 'IN / OUT', DATE_FORMAT(date, {date_format}) AS 'Time' FROM states, student WHERE states.personal = '0' AND states.id = student.student_id AND CONCAT(student.name, ' ', student.surname) REGEXP '{search}' ORDER BY date DESC ";
                this.last_page = (con.Counter(query) / limit) + 1;
                this.page_query = query + $" LIMIT {(current_page - 1) * limit}, {limit}";
                pages_lbl.Text = $"{current_page} / {last_page}";
                data_dgv.DataSource = con.SetDS(page_query).Tables[0];
